...
class DisplayDataCard(MDCard):
    ...
    edit_value=False
    #def on_enter(self):
       # self.getvalues(user_email.)
    def refresh(self,type):
        pass
    def change(self,tf):
        if tf==True:
            edit_value=True
            #DisplayDataCard.edit=True
    global user_email
    user_email=APP.load_current_acc()
    def getvalues(self,type):
        if type=='p':
            n,age,ph,addr=APP.get_pdtls(user_email)
            n=str(n)
            age=str(age)
            ph=str(ph)
            addr=str(addr)
            ...
            n,age,ph,addr,qlf,exp=APP.get_ddtls(user_email)
            n=str(n)
            age=str(age)
            ph=str(ph)
            addr=str(addr)
    ...
    def update(self,type):
        if type=='p':
            ...
            email=APP.load_current_acc()  #DisplayDataCard.user_name
    ...
    def check(self,type):
        ...
        if type=='p':
            if name!='' and age!='' and phn!='' and addr!='':
                APP.load_pdtls(email,name,age,phn,addr)
            ...
            if name!='' and age!='' and phn!='' and addr!='' and qlf!='' and exp!='':
                APP.load_ddtls(email,name,age,phn,addr,qlf,exp)
...
class MAINScreen(Screen):
    ...
    def values(self):
        ...
        for i in range(APP.get_total_doc()):
            if APP.get_doctor(i)==[]:
                ...
                #word_widget.ids.words.text = slowo
                card.add_widget(card_widget)
    def clear_before_leaving(self):
    ...
    def load_table(self):
        ...
        self.data_bp_tables = MDDataTable(
            ...
            sorted_order="DSC")
        #self.add_widget(self.data_bp_tables)
...
class AppoinBox(MDBoxLayout):
    ...
    def book_appoin(self):
        ...
        e=doc_selected
        print(t,d,e)
        #MainApp.msg('Your appoinment has been booked!')
        toast('Your appoinment has been booked!')
        #MainApp.msg('Your appoinment has been sent for confirmation!!')
...
class LoginCard(MDCard):
    app=MDApp.get_running_app()
    def redirect(scr):
        #self.root_widget.current= screen
        app=MDApp.get_running_app()
        app.root.current=scr #'home_screen'
...
#search_screens = {'Home': 'main.py', 'Settings': 'doctor.py', 'Appointments': 'home_screen'}
...
class MainApp(MDApp):
    ...
    dialog_bp=None
    def build(self):
        # loading screen animation duration change spot
        # ------------------------------------------------here-----------------------------------------
        #Clock.schedule_once(self.redirectt,4)
        self.root_widget = RootWidget()
        #screen_manager = ScreenManager()
        #screen_manager.add_widget(P_login_Screen(name='p_login_screen'))
        #other settings 
    ...
    def process(self,sname):
        #self.root.get_screen('main.py').ids.bmi_h.text
        sn=str(sname)
        print(sn)
        try:
            #emailid = self.root.get_screen(sn).ids.emailid.text
            #passwd = self.root.get_screen(sn).ids.passwd.text
            passwd=0
    ...
    def message(self,msg):
        self.snackbar = MDSnackbar(
            ...
            MDSnackbarCloseButton(
                ...
                text_color="#8E353C",
                _no_ripple_effect=True,
                on_release=MainApp.message_close(self),
            ),
            y=19,
...
if __name__ == "__main__":
    ...
    #except:
        ...
        #for i in range(5,0,-1):
         #   print('\rClosing the app in ',i,end='')
         #   sleep(0.5)
        #exit()
